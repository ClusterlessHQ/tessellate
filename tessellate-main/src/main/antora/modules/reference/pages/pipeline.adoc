...
<6> Whether to fail if rows don't meet the expected number of fields
<7> Partitions to parse into fields
<8> Transforms to apply to the data
<9> URL to write to, required
<10> Schema fields to declare, by default all fields are written
<11> Format type
<12> Compression type
<13> Whether the schema should be embedded in the files (add headers)
<14> Partitions to write out
