...
Def:::
- `^fixedWidth{width:...,indertAt:...} ->` - replace ALL fields with the fixed with result
- `^fixedWidth{width:...,indertAt:...} -> new_field|type + new_field2|type + etc` - name the new fields
Params:::
`width`:::: The width of the row/tuple.
`insertAt`:::: The index to begin inserting the null padding.
